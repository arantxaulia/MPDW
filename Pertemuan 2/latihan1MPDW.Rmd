...
data <- read_excel("C:/Users/arantxa/Downloads/Pertemuan 2/Arantxa.xlsx")
df <- data %>%
  mutate(
...
```
```{r}
dftime.ts <- ts(df$Tanggal)
dftime.ts
```
```{r}
dfprice.ts <- ts(df$Harga)
dfprice.ts
```
```{r}
summary(dftime.ts)
...
```
```{r}
ts.plot(dfprice.ts, xlab="Time Period ", ylab="Price", 
...
```
```{r}
plot(dfprice.ts, col="red",main="Plot semua data")
points(dfprice.ts)
```
```{r}
...
```
```{r}
ts.plot(dfprice.ts, xlab="Time Period ", ylab="Price", main= "SMA N=4 Data Saham")
points(dfprice.ts)
...
data.gab2 <- cbind(aktual = c(dfprice.ts), 
...
```
```{r}
ts.plot(dfprice.ts, xlab="Time Period ", ylab="Price", main= "DMA N=4 Data Saham")
points(dfprice.ts)
...
train.ts <- ts(training_ma$Harga)
test.ts  <- ts(testing_ma$Harga)
```
```{r}
plot(dfprice.ts, col="black",main="Plot semua data")
points(dfprice.ts)
```
```{r}
